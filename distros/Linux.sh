...
update_solace() {
    ...
    while true; do
        ...
        if [ "$CHOICE" = "Switch to Prebuilt" ] || [ "$CHOICE" = "Select Version" ]; then
            update_prebuilt; return
        fi
    done
}
...
while true; do
    ...
    UPTIME_TEXT=""
    if is_running; then UPTIME_TEXT=$(get_uptime); fi
