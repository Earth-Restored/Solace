...
public sealed class StringDictionaryValueComparer<TValue> : ValueComparer<Dictionary<string, TValue>>
    where TValue : class, IEquatable<TValue>, ICloneable<TValue>
{
...
#pragma warning disable CS8619 // Nullability of reference types in value doesn't match target type.
            a => a != null ? a.Select(item => item.Value == null ? item : new KeyValuePair<string, TValue>(item.Key, item.Value.DeepCopy())).ToDictionary() : new Dictionary<string, TValue>(StringComparer.Ordinal))
